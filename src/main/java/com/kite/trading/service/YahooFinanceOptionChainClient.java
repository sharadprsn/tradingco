...
public class YahooFinanceOptionChainClient implements OptionChainClient {
  ...
  private OptionChainData convert(final YahooResult result) {
      ...
      for (final YahooOptionGroup group : result.options()) {
        ...
        if (group.calls() != null) {
          for (final YahooContract yc : group.calls()) {
            ...
            data.add(
                new OptionData(
                    strike, expiryStr, toContract(yc, strike, expiryStr, underlying), pe));
          }
        }
        ...
        for (final Map.Entry<BigDecimal, OptionContract> entry : putsByStrike.entrySet()) {
          ...
          final boolean hasCe =
              group.calls() != null
                  && group.calls().stream()
                      .anyMatch(c -> BigDecimal.valueOf(c.strike()).compareTo(strike) == 0);
          if (!hasCe) {
    ...
    return new OptionChainData(new Records(expiryDates, data, null, underlying, strikes), null);
  }

  private OptionContract toContract(
      final YahooContract yc,
    ...
    return new OptionContract(
        ...
        change.compareTo(BigDecimal.ZERO) != 0 && oi.compareTo(BigDecimal.ZERO) > 0
            ? change
                ...
                .divide(oi.subtract(change), 2, RoundingMode.HALF_UP)
            : BigDecimal.ZERO,
  ...
  private static String formatExpiry(final long timestamp) {
    return Instant.ofEpochSecond(timestamp).atZone(ZoneId.of("Asia/Kolkata")).format(NSE_DATE_FMT);
  }

  @JsonIgnoreProperties(ignoreUnknown = true)
  private record YahooResponse(@JsonProperty("optionChain") YahooOptionChain optionChain) {}

  @JsonIgnoreProperties(ignoreUnknown = true)
  private record YahooOptionChain(@JsonProperty("result") List<YahooResult> result) {}
  ...
  @JsonIgnoreProperties(ignoreUnknown = true)
  private record YahooQuote(@JsonProperty("regularMarketPrice") double regularMarketPrice) {}
