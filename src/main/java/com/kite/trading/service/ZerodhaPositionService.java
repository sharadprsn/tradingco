 ...
 * 
 * This service handles position-related operations with Zerodha Kite API,
...
public class ZerodhaPositionService implements PositionService {
     ...
     */
    public ZerodhaPositionService(final ZerodhaApiClient apiClient, 
    ...
    public List<Position> getNiftyIntradayPositions() {
        ...
        final PositionsResponse response = fetchPositionsFromApi();
        final List<Position> dayPositions = response.data().day() != null
                ? response.data().day() : List.of();
    ...
    private List<Position> combinePositions(final PositionsResponse response) {
        final List<Position> netPositions = response.data().net() != null
                ? response.data().net() : List.of();
        final List<Position> dayPositions = response.data().day() != null
                ? response.data().day() : List.of();
        
        return dayPositions.stream()
                .filter(dayPos -> netPositions.stream()
                        .noneMatch(netPos -> netPos.tradingSymbol().equals(dayPos.tradingSymbol())
