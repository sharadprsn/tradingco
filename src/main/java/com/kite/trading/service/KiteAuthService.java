 ...
 *
 * <p>This interface defines the contract for the OAuth-based authentication flow with the Zerodha
 * Kite API:
 *
 * <ol>
 *   <li>Obtain a login URL via {@link #generateLoginUrl()}
 *   <li>User visits the URL in a browser, authenticates, and is redirected back with a {@code
 *       request_token}
 *   <li>Exchange the request token for an access token via {@link #generateSession(SessionRequest)}
 * </ol>
 *
 * This follows the Interface Segregation Principle (ISP) by keeping authentication concerns
 * separate from other API operations.
 *
...
public interface KiteAuthService {
   ...
   *
   * <p>The client should open this URL in a browser so the user can authenticate with their Zerodha
   * credentials (including TOTP). After successful login, Zerodha redirects to the configured
   * redirect URL with a {@code request_token} query parameter.
   *
   ...
   *
   * <p>This method calls the Kite API to generate an access token from the request token obtained
