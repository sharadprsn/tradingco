 ...
 *
 * <ol>
 *   <li>Call {@link #generateLoginUrl()} – returns a URL the user visits in their browser to log in
 *       with their Zerodha credentials + TOTP.
 *   <li>Zerodha redirects back with a {@code request_token} query parameter.
 *   <li>Call {@link #generateSession(SessionRequest)} – exchanges the request token for an access
 *       token that is used for all subsequent API calls. The request must include a {@code
 *       checksum} computed as the SHA-256 hex digest of {@code api_key + request_token +
 *       api_secret}.
 * </ol>
 *
 * This class follows the Single Responsibility Principle by handling only authentication concerns.
 *
...
public class ZerodhaAuthService implements KiteAuthService {
   ...
   */
  public ZerodhaAuthService(final ZerodhaApiClient apiClient, final KiteConfig kiteConfig) {
  ...
  public String generateLoginUrl() {
    final String url = kiteConfig.getLoginUrl() + "?api_key=" + kiteConfig.getApiKey() + "&v3=1";
  ...
  public KiteSession generateSession(final SessionRequest sessionRequest) {
    ...
    if (requestToken == null || requestToken.isBlank()) {
      throw new KiteAuthenticationException("request_token must not be null or blank");
    }
    ...
    final SessionResponse response =
        apiClient.generateSession(kiteConfig.getApiKey(), requestToken, checksum);

    if (response == null || response.data() == null) {
      throw new KiteAuthenticationException("Session generation returned an empty response");
    }

    currentSession = response.data();

    if (currentSession.accessToken() == null) {
      throw new KiteAuthenticationException("Session generated without an access token");
    }

    logger.info("Session generated successfully for user: {}", currentSession.userId());
   ...
   *
   * <p>The checksum is the lowercase hexadecimal SHA-256 digest of {@code api_key + request_token +
   * api_secret}.
   *
