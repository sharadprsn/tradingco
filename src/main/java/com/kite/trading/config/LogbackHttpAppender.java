...
/**
 * Custom Logback appender that forwards log events as JSON to a remote
 * HTTP endpoint.
 * 
 ...
 * }</pre>
 * 
 * <p>The appender builds a lightweight JSON payload without any additional
 * dependencies. It is designed to run under an {@link ch.qos.logback.classic.AsyncAppender
 * AsyncAppender} so that network I/O does not block the application thread.
 * 
 * <p>If the {@code url} is empty or {@code null} the appender is a no-op,
 * making it safe to keep in the configuration even when no endpoint is set.
 * 
...
public class LogbackHttpAppender extends AppenderBase<ILoggingEvent> {
    ...
    /**
     * Sets the target URL for log forwarding. When empty or {@code null}
     * {@link #append(ILoggingEvent)} becomes a no-op.
     *
        ...
        try {
            ...
            if (responseCode < 200 || responseCode >= 300) {
                addStatus(new ErrorStatus(
                        "HTTP log endpoint returned " + responseCode + " for URL: " + url,
                        this));
            }
            ...
            addStatus(new ErrorStatus(
                    "Failed to send log event to URL: " + url + " - " + e.getMessage(),
                    this));
        }
    }
    ...
    private String buildJsonPayload(final ILoggingEvent event) {
        ...
        appendString(json, "timestamp", TIMESTAMP_FORMATTER.format(
        ...
        if (layout != null) {
            ...
            formattedMessage = new String(
                    encoder.encode(event), StandardCharsets.UTF_8);
        } else {
     ...
     */
    private static void appendString(final StringBuilder json,
                                     final String key,
                                     final String value) {
        json.append('"');
        json.append(key);
